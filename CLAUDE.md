...
- Supports: `xiaohongshu`, `douyin` (legacy), `douyin_new` (Playwright+CDP)
...
- **Crawling Speed**: Browser automation is inherently slow. Deep crawl (with comments) is slower but richer.
...
- **Server Deployment**: Set `HEADLESS=true` for production environments.
